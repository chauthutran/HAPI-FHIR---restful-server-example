...
import ca.uhn.example.model.MyPatient;
...
/**
 * This is a resource provider which stores MyPatient resources in memory using a HashMap. This is obviously not a production-ready solution for many reasons, 
...
public class PatientResourceProvider implements IResourceProvider {
	 ...
	 */
	private Map<Long, Deque<MyPatient>> myIdToPatientVersions = new HashMap<Long, Deque<MyPatient>>();
	...
	public PatientResourceProvider() {
		...
		patient.setBillingCode(new CodeDt("00102-1"));
		patient.setPetName(new StringDt("PET NAME 1"));
		
		
		LinkedList<MyPatient> list = new LinkedList<MyPatient>();
		list.add(patient);
	 ...
	 */
	private void addNewVersion(MyPatient thePatient, Long theId) {
		InstantDt publishedDate;
		if (!myIdToPatientVersions.containsKey(theId)) {
			myIdToPatientVersions.put(theId, new LinkedList<MyPatient>());
			publishedDate = InstantDt.withCurrentTime();
		} else {
			MyPatient currentPatitne = myIdToPatientVersions.get(theId).getLast();
		...
		Deque<MyPatient> existingVersions = myIdToPatientVersions.get(theId);

		// We just use the current number of versions as the next version number
		String newVersion = Integer.toString(existingVersions.size());
		
		// Create an ID with the new version and assign it back to the resource
		IdDt newId = new IdDt("MyPatient", Long.toString(theId), newVersion);
	 ...
	 */
	@Create()
	public MethodOutcome createPatient(@ResourceParam MyPatient thePatient) {
	 ...
	 */
	@Search()
	public List<MyPatient> findPatientsByName(@RequiredParam(name = MyPatient.SP_FAMILY) StringDt theFamilyName) {
		LinkedList<MyPatient> retVal = new LinkedList<MyPatient>();

		/*
		 * Look for all patients matching the name
		 */
		for (Deque<MyPatient> nextPatientList : myIdToPatientVersions.values()) {
			MyPatient nextPatient = nextPatientList.getLast();
	...
	@Search
	public List<MyPatient> findPatientsUsingArbitraryCtriteria() {
		LinkedList<MyPatient> retVal = new LinkedList<MyPatient>();
	 ...
	 */
	@Override
	public Class<MyPatient> getResourceType() {
	 ...
	 */
	@Read()
	public MyPatient readPatient(@IdParam IdDt theId) {
		Deque<MyPatient> retVal;
		try {
		...
		if (theId.hasVersionIdPart() == false) {
			...
			for (MyPatient nextVersion : retVal) {
	 ...
	 */
	@Update()
	public MethodOutcome updatePatient(@IdParam IdDt theId, @ResourceParam MyPatient thePatient) {
		validateResource(thePatient);
	 ...
	 */
	private void validateResource(MyPatient thePatient) {
		/*
		...
		if (thePatient.getNameFirstRep().getFamilyFirstRep().isEmpty()) {
			...
			outcome.addIssue().setSeverity(IssueSeverityEnum.FATAL).setDetails("No family name provided, MyPatient resources must have at least one family name.");
